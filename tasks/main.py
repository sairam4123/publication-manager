...
client = create_client("https://xchmpfivomtlnslvbbbk.supabase.co", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhjaG1wZml2b210bG5zbHZiYmJrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MjQ2MDI4MDYsImV4cCI6MjA0MDE3ODgwNn0.HBs-xlJW21awsjyS5mje25g3Wu5M_TGFc2T7Q6urXLw")
...
def process_excel(in_file: str):
    ...
    with io.BytesIO() as in_buffer, io.BytesIO() as out_buffer:
        in_buffer.write(client.storage.from_("excel-storage").download(in_file))
        asyncio.run(task_dblp(in_buffer, out_buffer))
        client.storage.from_("excel-storage").upload(out_file, out_buffer.getvalue()) # upload requires bytes not bytesIO
    return out_file
...
def process_excel_storage(in_filename: str):
    ...
    with open(abs_in_path, "wb") as buffer:
        buffer.write(client.storage.from_("excel-storage").download(in_filename))

    asyncio.run(task_dblp(abs_in_path, abs_out_path))
    client.storage.from_("excel-storage") \
